class TestSystem < Minitest::Test
  ...
  def test_system_new
    ...
    s = System::new([42, 28, 23], [BC::Free, BC::Per, BC::Free], :s0)
    assert_equal([42, 28, 23], s.reference_dimensions)
  ...
  def test_dimensions
    ...
    s = System::new([42, 28, 23], [BC::Free, BC::Per, BC::Free], :s0)
    assert_equal([42, 28, 23], s.dimensions(:s0))
    assert_equal([42 - 7, 28, 23 - 7], s.dimensions(:s1))
    assert_equal([42*2 + 15, 28*2, 23*2 + 15], s.dimensions(:r))
  end
  ...
  def test_shapes
    ...
    s = System::new([42, 28, 23], [BC::Free, BC::Per, BC::Free], :s0)
    assert_equal([2, 42, 2, 28, 2, 23 + 1], s.shapes(:s0))
    assert_equal([42 - 7 + 1, 2, 28, 2, 23 - 7, 2], s.shapes(:s1))
    assert_equal([42*2 + 15 + 1, 28*2, 23*2 + 15 + 3], s.shapes(:r))
    assert_equal([42 - 7 + 1, 2, 2, 28, 23*2 + 15 + 3], s.shapes([:s1, :s0, :r]))
  end
  ...
  def test_leading_dimensions
    ...
    s = System::new([42, 28, 23], [BC::Free, BC::Per, BC::Free], :s0)
    assert_equal([42, 28, 23 + 1], s.leading_dimensions(:s0))
    assert_equal([42 - 7 + 1, 28, 23 - 7], s.leading_dimensions(:s1))
    assert_equal([42*2 + 15 + 1, 28*2, 23*2 + 15 + 3], s.leading_dimensions(:r))
    assert_equal([42 - 7 + 1, 28, 23*2 + 15 + 3], s.leading_dimensions([:s1, :s0, :r]))
  end
